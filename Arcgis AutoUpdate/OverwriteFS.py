...
def _asyncJob( service, endpoint, data, verbose=None, indent="", noWait=False):
...
"""
    ...
    try:
        ...
        if managerProperties:
            ...
            managerProperties[ "layers"] = [dict( layer.properties) for layer in manager.layers]
            managerProperties[ "tables"] = [dict( table.properties) for table in manager.tables]
...
"""
    ...
    if not noProps:
        ...
        if hasattr( manager, "properties"):
            ...
            for title, array, backup in [["Layer", manager.layers, layerProperties], ["Table", manager.tables, tableProperties]]:
                for index, layer in enumerate( array):
                    ...
                    if verbose:
                        print( "\nRestoring {} Properties for: '{}'{}".format( title, layerName, " * Optimized Layer *" if multiScaleGeometry else ""))
                        ...
                        if not isView:
                            ...
                            for index in serviceDefinition[ "indexes"]:
                                for newLayerIndex in layer.properties[ "indexes"]:
                                    if index[ "fields"] == newLayerIndex[ "fields"]:
                                        break
                                else:
                                    skipMessage = ""
                                    indexFields = []
...
def _importConverter( moduleName):
    ...
    for folder in [filePath] + [os.path.join( filePath, modPath) for modPath in moduleName.split(".")[:-1]]:
...
def getFeatureServiceTarget( view, verbose=None, outcome=None):
        ...
        if not (hasattr( viewManager, "properties") and hasattr( viewManager.properties, "isView") and viewManager.properties.isView):
            ...
            if not len( views) == 2:
                ...
                for relatedView in views:
                    ...
                    if not fileItem:
                        outcome[ "items"].append( {"id": serviceItem.id, "title": serviceItem.title, "itemType": serviceItem.type, "action": "verify", "result": "Target Feature Service is missing a file Data Source"})
                        outcome[ "success"] = False     # Set as error
                    else:
                        targets.append( {"view": relatedView, "service": serviceItem, "filename": fileItem[0].name, "fileType": fileItem[0].type})
...
def swapFeatureViewLayers( view, updateFile=None, touchItems=True, verbose=None, touchTimeSeries=True, outcome=None, noIndexes=False, preserveProps=True, noWait=False, noProps=False, converter=None, outPath="", dryRun=False):
    """Function: swapFeatureViewLayers( <view>[, <updateFile>[, <touchItems>[, <verbose>[, <touchTimeSeries>[, <outcome>[, <noIndexes>[, <preserveProps>[, <noWait>[, <noProps>[, <converter>[, <outPath>[, <dryRun>]]]]]]]]]]]])
...
"""
    ...
    if converter:
        ...
        if isinstance( converter[0], str):
            try:
                converter[0] = _importConverter( converterFolder + "." + converter[0])
    ...
    #
    target = getFeatureServiceTarget( view, verbose=verbose, outcome=outcome)
    ...
    if "service" in target:
        ...
        if (updateFile and outcome[ "success"] == True) or not (updateFile or outcome[ "success"] == False):
            ...
            fallbackLayers = {"layers": viewManager.properties["layers"][:]}

            # Collect Administrative details for All availble Layers
            for layer in targetManager.properties["layers"]:
            ...
            layers = viewManager.properties["layers"]
...
def overwriteFeatureService( item, updateFile=None, touchItems=True, verbose=None, touchTimeSeries=True, outcome=None, ignoreItems=[], serviceLastModified=0, noIndexes=False, preserveProps=True, noWait=False, noProps=False, converter=None, outPath="", dryRun=False):
...
"""
    ...
    if not item.type == "Feature Service":
        ...
        if updateFile:
            ...
            #
            hasChangeTrackingOnView = manager.properties.get( "hasChangeTrackingEnabledViews", False)
            ...
            if updateFile.split(":")[0].lower() in ["ftp", "http", "https"]:
                ...
                try:
                    ...
                    if verbose:
                        ...
                        print( " - Last Modified: {}".format( fileLastModified.strftime( "%a, %d %b %Y %H:%M:%S GMT") if fileLastModified else "N/A"))
                        if maxVerbose:
                            print( "\nWeb Headers:")
                    ...
                    try:
                        ...
                        elif not "Last-Modified" in headers:
                            # Trigger File comparison if no Last-Modified header provided and we have an existing download!
                            if fileLastModified:
                ...
                if not os.path.exists( updateFile):
                    outcome[ "items"].append( {"id": item.id, "title": item.title, "itemType": item.type, "action": "verify", "result": "Update Filename '{}', Originally Published with Service, CANNOT be found in Folder: '{}'".format( outputFile, os.path.split( updateFile)[0])})
                    outcome[ "success"] = False     # Set as error
            ...
            if converter:
                ...
                updatePath, updateFilename = os.path.split( os.path.realpath( updateFile))  # Extract Path and Filename
                ...
                try:
                    ...
                    if "verbose" in argNames and (argNames.index( "verbose") + 1) > len( args): # If 'verbose' available and not in arguments provided, add it!
                        updateFile = converter.convert( updateFile, *args, verbose=(True if verbose else False))
                    else:
            ...
            #
            # Check if we need to compare Downlaod to last file
            #
            if lastFile:
            ...
            try:
                #
                # Drop Layer Optimizations
                #
                for layer in manager.layers:
                    if "multiScaleGeometryInfo" in layer.properties:
                        if verbose:
                            print( "   - Dropping Optimization on Layer Id: {}".format( layer.properties.get( "id", "")))
                        start = datetime.datetime.now()
                        if dryRun:
                            if verbose:
                                print( " * Dry Run * No Change!")
                            ...
                            #data = { "f": "json", "async": False, "updateDefinition": '{"multiScaleGeometryInfo":null}'}
                            #status = layer._gis._con.post(  layer.url + "/" + "updateDefinition", data)
                            status = _asyncJob( layer, "updateDefinition", { "f": "json", "async": True, "updateDefinition": '{"multiScaleGeometryInfo":null}'}, verbose=True if maxVerbose else (None if verbose else False), indent="    ")

                        if not (isinstance( status, dict) and status.get( "success", False)):
                            if not verbose == False:
                                print( "\n * Failed! Error: {}".format( status))
                        elif verbose:
                            _prints( "\r   - Optimization Dropped! Elapsed Time: {}    ".format( datetime.datetime.now() - start), status.get( "progressLength", 0) if hasattr( status, "get") else 0)
...
if __name__ == "__main__":
    ...
    if len( sys.argv) < 4 or help:
        print( "\n{} Usage: Python {} [-h] <profile> <item> <title> [<filename> | <url>] [-OutPath <output folder>] [-NoTimeSeries] [-NoIndexes] [-NoTouch] [-NoWait] [-NoProps | -PersistProps] [-DryRun] [-GetTarget | -SwapLayers | [-ListRelated] [-AddRelated | -RemoveRelated [<Item A id>[ <Item B id>]]]] [-Convert <module>[ <call param>[ <call param>[ ...]]]] [-AllowPWprompt] [-LessDetail | -MoreDetail] [-Password <password>]".format( version, __file__))
        ...
        print( "                           Optimization to complete before continuing the Overwrite or Layer Swap action. When enabled,")
        print( "                           function will report condition and supply a URL that can be used for manual status review.")
        print( "                           Default: Function will wait for properties like Layer Optimization to be re-applied before")
        print( "                                    proceeding to the next processing 'step' in the Overwrite or Layer Swap workflow.")
        ...
        print( "                           or newly Overwritten Feature Service. Used by A/B Feature Service enabled View, whereby")
        print( "                           the View is Related to Two Feature Services, allowing the View's Layers to be swapped")
        print( "                           out to point to the matching Layers of the newly updated Feature Service.")
        ...
        print( "                           and File Item upload Filename that should be used for the next update target.")
        print( "                           Leveraged by A/B View enabled Services to select inactive")
        print( "                           Feature Service that should be overwritten next.")
        ...
        print( "                           transform the source data before Overwriting the Hosted Feature Service.")
        ...
        print( "    <password>: (optional, but required when using -Password Option)")
    ...
    validSwitches = [
        ["SwapLayers", True], ["NoTimeSeries", True], ["NoIndexes", True], ["NoTouch", True], ["NoWait", True], ["NoProps", True], ["PersistProps", True], ["GetTarget", True], ["ListRelated", True],
        ["Async", True], ["AllowPWprompt", True], ["AddRelated", []], ["RemoveRelated", []], ["LessDetail", True], ["MoreDetail", True], ["DryRun", True], ["Password", ""], ["OutPath", ""], ["Convert", []]
    ]
    lowerSwitches = {}
    localVars = locals()
    ...
    if gettarget:
        ...
        swapFeatureViewLayers( item, updateFile=updateFile, verbose=verbose, touchTimeSeries=not notimeseries, outcome=outcome, noIndexes=noindexes, preserveProps=persistprops, noWait=nowait, noProps=noprops, converter=convert, outPath=outpath, dryRun=dryrun)
        print( "\nElapsed Time for Swap Layers Process: {}".format( datetime.datetime.now() - start))
        ...
        # Do Overwrite
        overwriteFeatureService( item, updateFile=updateFile, touchItems=not notouch, verbose=verbose, touchTimeSeries=not notimeseries, outcome=outcome, noIndexes=noindexes, preserveProps=persistprops, noWait=nowait, noProps=noprops, converter=convert, outPath=outpath, dryRun=dryrun)
